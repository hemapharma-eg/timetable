...
export function Benchmarking({ initialPage = 'dashboard' }) {
  ...
  const downloadPowerPoint = () => {
    ...
    titleSlide.addText("Institutional Benchmarking Report", { 
      x: 1, y: 2.5, w: 11.33, h: 1.5, 
      fontSize: 44, bold: true, color: '1E293B', align: 'center',
      fontFace: 'Arial'
    });
    
    titleSlide.addText(`Performance Matrix • ${currentYear?.name || ''} • ${new Date().toLocaleDateString()}`, { 
    ...
    dashboardData.forEach(d => {
      ...
      slide.addText(d.definition.category.toUpperCase(), { 
        x: 0.5, y: 0.15, w: 12.33, h: 0.2, 
        fontSize: 10, color: '6366F1', bold: true, tracking: 2 
      });
      
      slide.addText(d.definition.name, { 
        x: 0.5, y: 0.35, w: 12.33, h: 0.4, 
        fontSize: 24, color: '1E293B', bold: true 
      });

      // Data Visualization (Chart)
      const activeUnis = universities.filter(u => u.active);
      const labels = [];
      const values = [];
      
      activeUnis.forEach(uni => {
        const val = d.values[uni.id];
        labels.push(uni.abbr);
        // Try to parse numeric value for chart, handle ranges like "251-300"
        let numVal = parseForChart(val);
        if (numVal === null && val && val.includes('-')) {
          const parts = val.split('-').map(p => parseForChart(p));
          if (parts[0] !== null) numVal = parts[0]; // Use lower bound for ranking
        }
        values.push(numVal || 0);
      });

      // Add Chart (Left Side)
      slide.addChart(pptx.ChartType.bar, 
        [{ name: d.definition.name, labels: labels, values: values }], 
        { 
          x: 0.5, y: 1.5, w: 8.5, h: 5.0,
          showValue: true,
          valueFontSize: 12,
          barGapWidthPct: 30,
          chartColors: ['4338CA'],
      ...
      // Strategic Action Plan (Right Side - Narrower)
      const plans = benchmarkingActionPlans.filter(p => p.kpi_id === d.kpiId && p.year_id === d.yearId);
      if (plans.length > 0) {
        slide.addShape(pptx.ShapeType.rect, { 
          x: 9.3, y: 1.5, w: 3.5, h: 5.0, 
          fill: { color: 'F0FDF4' }, 
          line: { color: 'BBF7D0', width: 1 } 
        });
        
        slide.addText("STRATEGIC ACTION PLAN", { 
          x: 9.4, y: 1.7, w: 3.3, h: 0.4, 
        ...
        plans.slice(0, 5).forEach((plan) => {
          // Action Text
          slide.addText(`• ${plan.action_text}`, { 
            x: 9.4, y: currentY, w: 3.3, h: 0.8, 
            fontSize: 11, color: '14532D',
            valign: 'top'
          });
          currentY += 0.85;
          
          // Status (Avoid overlapping)
          slide.addText(`Status: ${plan.status.toUpperCase()}`, { 
            x: 9.6, y: currentY - 0.2, w: 3.1, h: 0.2, 
            fontSize: 9, color: '166534', bold: true, italic: true 
          });
          currentY += 0.35;
        });
      }
      ...
      // Footer
      slide.addText(`DMU Quality Assurance Hub • Page ${pptx.slides.length}`, { x: 0.5, y: 7.1, w: 12.33, h: 0.2, fontSize: 9, color: 'CBD5E1', align: 'right' });
    });

    pptx.writeFile({ fileName: `Benchmarking_Report_${currentYear?.name || 'Report'}.pptx` });
  };
    ...
    <div className="min-h-full bg-transparent font-sans text-gray-900 flex flex-col print:bg-white animate-in fade-in duration-500">
      ...
      <div className="mb-8 flex justify-between items-center bg-white p-6 rounded-3xl border border-slate-100 shadow-sm print:hidden">
        ...
        <div className="flex items-center gap-3">
            <button onClick={downloadPowerPoint} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 transition-all">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div className="grid grid-cols-1 gap-4">
              {universities.map(uni => (
                <div key={uni.id} className="bg-white border border-slate-100 rounded-[32px] p-6 shadow-sm hover:shadow-md transition-all duration-300 flex items-center justify-between gap-8">
                  <div className="flex items-center gap-6">
                    <div className="w-16 h-16 bg-slate-50 text-indigo-600 rounded-2xl flex-shrink-0 flex items-center justify-center font-black text-sm border border-slate-100 shadow-inner">
                      {uni.abbr}
                    </div>
                  </div>

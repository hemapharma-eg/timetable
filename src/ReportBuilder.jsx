...
import {
  ...
  GripVertical, ChevronDown, ChevronUp, Filter, ArrowUpDown, Copy, Save
} from 'lucide-react';
...
// ─── Saved Reports persistence via localStorage ───────────────────────────────
const STORAGE_KEY = 'cms_saved_reports';
...
export const ReportBuilder = ({ faculty, students, courses }) => {
  ...
  useEffect(() => { persistReports(savedReports); }, [savedReports]);
  ...
  const loadReport = (report) => {
    ...
    setSortKey(report.sortKey || '');
    setSortDir(report.sortDir || 'asc');
    setGroupByKey(report.groupByKey || '');
  ...
  const saveCurrentReport = () => {
    ...
    if (editingReportId) {
      setSavedReports(prev => prev.map(r => r.id === editingReportId ? reportObj : r));
    } else {
      setSavedReports(prev => [...prev, reportObj]);
      setEditingReportId(reportObj.id);
    }
  };

  const duplicateReport = (report) => {
    const clone = { ...report, id: Date.now().toString(), name: report.name + ' (Copy)', updatedAt: new Date().toISOString() };
    setSavedReports(prev => [...prev, clone]);
  };
          ...
          {savedReports.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {savedReports.map(report => {
                const src = DATA_SOURCES[report.dataSource];
                return (
                  <div key={report.id} className="group border border-slate-200 rounded-xl p-5 hover:border-violet-300 hover:shadow-md transition-all bg-white">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold bg-${src?.color || 'slate'}-100 text-${src?.color || 'slate'}-700`}>
                        {src?.label || report.dataSource}
                      </span>
                      <span className="text-xs text-slate-400">{report.columns.length} columns</span>
                      {report.filters?.length > 0 && (
                        <span className="text-xs text-slate-400">· {report.filters.length} filter(s)</span>
                      )}
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 min-h-[500px]">
            ...
            <div className="p-4 border-b border-slate-200 bg-slate-50">
              ...
              <div className="flex gap-2">
                {Object.entries(DATA_SOURCES).map(([key, src]) => (
                  <button
                    key={key}
                    onClick={() => { setDataSource(key); setSelectedColumns([]); setFilters([]); setSortKey(''); setGroupByKey(''); }}
                    className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all ${dataSource === key
          ...
          <div className="flex flex-col overflow-hidden lg:col-span-2">
            ...
            <div className="p-4 border-b border-slate-200">
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block">Column Order (drag to reorder)</label>
              {selectedColumns.length === 0 ? (
            ...
            <div className="p-4 border-b border-slate-200 flex flex-wrap gap-4 items-end">
              <div className="flex-1 min-w-[200px]">
                ...
                <select value={sortKey} onChange={e => setSortKey(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm outline-none focus:ring-1 focus:ring-violet-400 bg-white">
                  ...
                  {selectedColumns.map(k => {
                    const fld = allFields.find(f => f.key === k);
                    return <option key={k} value={k}>{fld?.label || k}</option>;
                  })}
                </select>
              </div>
              {sortKey && (

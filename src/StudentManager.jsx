...
import { Upload, Plus, Pencil, Trash2, X, Search, GraduationCap, CheckSquare } from 'lucide-react';
...
export const StudentManager = ({ students, setStudents, isReadOnly = false }) => {
  ...
  const toggleSelectAll = () => { if (allVisibleSelected) setSelectedIds(new Set()); else setSelectedIds(new Set(filteredStudents.map(s => s.id))); };
  const handleBulkDelete = () => { if (!confirm(`Delete ${selectedIds.size} selected record(s)?`)) return; setStudents(students.filter(x => !selectedIds.has(x.id))); setSelectedIds(new Set()); };
    ...
    reader.onload = (evt) => {
      try {
        ...
        for (let i = 1; i < data.length; i++) {
          ...
          STUDENT_FIELDS.forEach((f) => { const hIdx = headers.indexOf(f.key); if (hIdx >= 0 && row[hIdx] !== undefined) item[f.key] = row[hIdx].toString().trim(); });
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-slate-100 border-b border-slate-200 text-slate-700 font-medium text-sm">
                ...
                <th className={`p-3 ${isReadOnly ? 'pl-4' : ''} sticky left-0 bg-slate-100 z-10 w-64`}>Name / ID</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredStudents.map(s => (
                <tr key={s.id} className={`hover:bg-slate-50 transition-colors group ${selectedIds.has(s.id) ? 'bg-indigo-50/50' : ''}`}>
                  {!isReadOnly && <td className="p-3 pl-4"><input type="checkbox" checked={selectedIds.has(s.id)} onChange={() => toggleSelect(s.id)} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" /></td>}
              ...
              {filteredStudents.length === 0 && (
                <tr>
                  <td colSpan={isReadOnly ? 6 : 8} className="p-8 text-center text-slate-500">

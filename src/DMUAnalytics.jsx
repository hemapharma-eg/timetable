...
const CorrectiveActionPlan = ({ session, userMeta, dashboardData, programs, onCalculateTrend, prefillData, onPrefillClear }) => {
  const isAdmin = userMeta?.role === 'technical_admin' || userMeta?.role === 'academic_admin' || session?.user?.email === 'dribrahimpharmaceutics@gmail.com';
  ...
  const exportToExcel = () => {
    ...
    const dataRows = targetCaps.map(cap => {
      ...
      if (isAdmin) {
        rowObj['Created By'] = cap.created_by || 'Unknown';
      }
      ...
      if (isAdmin) {
        html += `
            <tr>
              ...
              <td class="field-value"><b>${cap.created_by || 'Unknown'}</b></td>
            </tr>
        `;
      }
        ...
        <div className="bg-white px-8 py-6 border-b border-slate-100 flex justify-between items-center shrink-0 shadow-sm relative z-10">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              title={isAdmin ? "Export all action plans to Excel" : "Export my action plans to Excel"}
            >
              <Download size={14} /> {isAdmin ? 'Export All Plans (Excel)' : 'Export My Plans (Excel)'}
            </button>
            <button
              ...
              title={isAdmin ? "Export all action plans to Word" : "Export my action plans to Word"}
            >
              <FileText size={14} /> {isAdmin ? 'Export All Plans (Word)' : 'Export My Plans (Word)'}
            </button>
          </div>
        </div>

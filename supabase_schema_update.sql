...
ALTER TABLE public.risk_management_plan ADD COLUMN IF NOT EXISTS "Risk_Owner" text;
ALTER TABLE public.risk_management_plan ADD COLUMN IF NOT EXISTS "Risk_Reporter" text;
ALTER TABLE public.risk_management_plan ADD COLUMN IF NOT EXISTS "Impact" numeric;
ALTER TABLE public.risk_management_plan ADD COLUMN IF NOT EXISTS "Appetite" numeric;
ALTER TABLE public.risk_management_plan ADD COLUMN IF NOT EXISTS "Mitigating_Actions" text;

CREATE TABLE IF NOT EXISTS public.kri_rubrics (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  kri_id bigint, -- or uuid depending on risk_kris, fallback to text if unknown
  rubric_value text,
  likelihood numeric,
  created_at timestamptz DEFAULT now()
);
ALTER TABLE public.kri_rubrics ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow full access on kri_rubrics" ON public.kri_rubrics;
CREATE POLICY "Allow full access on kri_rubrics" ON public.kri_rubrics FOR ALL USING (true) WITH CHECK (true);
